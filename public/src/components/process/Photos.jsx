...
const Photos = () => {
  ...
  return (
    <div className="flex gap-5 items-center justify-center flex-col h-full">
      ...
      <CldUploadButton options={{multiple: true}}
        onUpload={handleUpload}
        uploadPreset="td0bg1jq">
          <span className="bg-d3prop-gradient py-3 mt-5 px-5 text-white text-base font-medium rounded-md cursor-pointer">
            Upload
          </span>
        </CldUploadButton>
        <div className="grid grid-cols-3 gap-4 h-[55vh] overflow-auto pb-10 no-scrollbar">
          {
            photos.map((photo) => (
              <div className="relative h-36 w-[200px]" key={photo}>
                <Image src={photo} fill alt="Upload"></Image>
              </div>))
          }
        </div>
    </div>
